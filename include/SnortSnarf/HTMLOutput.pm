...
sub BEGIN {
    %defaults= ( # default 'new' params
        ...
        'output_dir' => undef, # base output directory
        'dnslookupnet' => undef, # do DNS lookup if true, but only in cidr specified net
        ...
        'onewindow' => 0, # if true, keep all pages in one browser window
        'mostsigfirst' => 0, # if true, sort the signature index page to show the signatures with the most alerts first
        'sortsigcountfirst' => 0, # if true, sort the sig index list so that # of alerts is considered more important than priority
        'hiprioisworse' => 0, # if true, alerts with a higher priority # are considered more severe
        ...
        'foot' => '', # footer text
        'gmt' => 0, # packets times are GMT print local times on pages
        'ymd' => 0, # list date in year/month/date format
    ...
    %ICMP_text_to_filename= ( 
        ...
        'DESTINATION UNREACHABLE: FRAGMENTATION NEEDED, DF SET' => 'ICMP_UNRCH_FRAG_NEEDED', 
        ...
        'DESTINATION UNREACHABLE: NET UNREACHABLE FOR TOS' => 'ICMP_UNRCH_NET_UNR_TOS', 
        'DESTINATION UNREACHABLE: HOST UNREACHABLE FOR TOS' => 'ICMP_UNRCH_HOST_UNR_TOS', 
        'DESTINATION UNREACHABLE: PACKET FILTERED' => 'ICMP_UNRCH_PACKET_FILT', 
        'DESTINATION UNREACHABLE: ADMINISTRATIVELY PROHIBITED,' => 'ICMP_UNRCH_PACKET_FILT', 
        'DESTINATION UNREACHABLE: ADMINISTRATIVELY PROHIBITED, PACKET FILTERED' => 'ICMP_UNRCH_PACKET_FILT', 
        'DESTINATION UNREACHABLE: ADMINISTRATIVELY PROHIBITED PACKET FILTERED' => 'ICMP_UNRCH_PACKET_FILT', 
        'DESTINATION UNREACHABLE: ADMINISTRATIVELY PROHIBITED NETWORK FILTERED' => 'ICMP_UNRCH_PKT_FILTERED_NET', 
        'DESTINATION UNREACHABLE: ADMINISTRATIVELY PROHIBITED HOST FILTERED' => 'ICMP_UNRCH_PKT_FILTERED_HOST', 
        ...
        'REDIRECT NET' => 'ICMP_REDIRECT',
        'REDIRECT HOST' => 'ICMP_REDIRECT', 
        'REDIRECT TOS NET' => 'ICMP_REDIRECT', 
        'REDIRECT TOS HOST' => 'ICMP_REDIRECT', 
        'ECHO' => 'ICMP_ECHO', 
        'ROUTER ADVERTISMENT' => 'ICMP_UNKNOWN', 
        'ROUTER SOLICITATION' => 'ICMP_UNKNOWN', 
        'TTL EXCEEDED' => 'ICMP_TTL_EXCEED', 
        'TTL EXCEEDED IN TRANSIT' => 'ICMP_TTL_EXCEED',
        'TTL EXCEEDED TIME EXCEEDED IN FRAG REASSEMBLY' => 'ICMP_TTL_EXCEED', 
        'PARAMETER PROBLEM' => 'ICMP_PARAM_PROB', 
        'PARAMETER PROBLEM: BAD IP HEADER BYTE' => 'ICMP_PARAM_PROB',
        'PARAMETER PROBLEM: OPTION MISSING' => 'ICMP_PARAM_PROB', 
        'PARAMETER PROBLEM: BAD LENGTH' => 'ICMP_PARAM_PROB', 
    ...
    $summbgcol= '#FFFFCC'; # summary background color

    $logo_filename= 'SDlogo.gif';
    
    $fhnonce='HOfh00';
    
    # prioritization for what to list first in reference links; the first one also is the link under the signature text
    %reflistorder= (
        'url' => 1, # #1 since probably most specific
        'sid' => 2,
        'bugtraq' => 3,
        'cve' => 4,
        'mcafee' => 5,
...
sub new {
    ...
    if (defined($homenet) && $homenet !~ /^\s*$/) {
        ($params{'homenetaddr'}, $params{'homenetmask'}) = check_cidr('homenet', $homenet);
    }

    # set up dnsnet mask and address parameters
    my $dnsnet = delete $params{'dnslookupnet'};
    if( defined($dnsnet ) && $dnsnet !~ /^\s*$/) {
        ($params{'dnsaddr'}, $params{'dnsmask'}) = check_cidr('dns', $dnsnet);
    }
...
sub check_cidr {
    ...
    my($addr,$bits)= split('/',$net);
    my(@bytes)= split('\.',$addr);
    warn "HTMLOutput: $addr does not seem to be a valid address, but proceeding\n" if (@bytes <= 1 || @bytes > 4);
    unless (defined($bits)) {
        if ($addr =~ /^0\.0\.0\.0$/) {
            $bits= 0;
        } elsif ($addr =~ /\.0\.0\.0$/) {
    ...
    my $mask= &bytenums2bits(@bytembits);
    $addr &= $mask;
    return ($addr, $mask);
}
...
sub output {
    ...
    # move the 3 defined parameters to 'cur'
    $self->{'cur'}{'insources_str'}= $params->{'insources_str'}; # encoded input recreation string to be given to external scripts
    $self->{'cur'}{'insources_str_noinfilter'}= $params->{'insources_str_noinfilter'}; # encoded input recreation string to be given to external scripts, but without input filter
    ...
    foreach (qw(foot prog_line mostsigfirst topquant sortsigcountfirst hiprioisworse)) { # copy params over from "new" params unless provided
        $self->{'cur'}{$_}= defined($params->{$_}) ? $params->{$_} : $self->{$_};
    }
    # set up the output directory paths
    my $outdir= (defined($params->{'outdir'}) ? $params->{'outdir'} : $self->{'output_dir'});
    my $dirsep= $self->{'dirsep'};
    unless (defined($outdir)) {
        my $dirsepre= '\\'.$dirsep;
        $repr_insource =~ /([^$dirsepre]+)$/; # extract file name from path
        $outdir= "snfout.".$1;
    }
    $self->{'cur'}{'g'}{'outdir'}= $outdir.$dirsep.$self->sig_sect_dir();
    $self->{'cur'}{'a'}{'outdir'}= $outdir.$dirsep.$self->anom_sect_dir();
    $self->{'cur'}{'s'}{'outdir'}= $outdir.$dirsep.$self->summ_sect_dir();
    ...
    if (@astores) {
        if (@astores > 1) {
            $astores[0]= MultiStore->new('cont' => [@astores]); # we are not adding, so don't care in what order the stores are in
        }
        $self->{'cur'}{'a'}{'store'}= $self->{'cur'}{'all'}{'store'}= $astores[0];
    } else {
        $self->{'cur'}{'a'}{'store'}= undef;
    }
    if (@gstores) {
        if (@gstores > 1) {
            $gstores[0]= MultiStore->new('cont' => [@gstores]); # we are not adding, so don't care in what order the stores are in
        }
        $self->{'cur'}{'g'}{'store'}= $self->{'cur'}{'all'}{'store'}= $gstores[0];
    } else {
        $self->{'cur'}{'g'}{'store'}= undef;
    }
    if (@astores && @gstores) {
        $self->{'cur'}{'all'}{'store'}= MultiStore->new('cont' => [$self->{'cur'}{'a'}{'store'},$self->{'cur'}{'g'}{'store'}]); # we are not adding, so don't care in what order the stores are in
    }
...
sub gen_sig_pages {
    ...
    $self->{'cur'}{'topbase'}= &sig_sect_basepath();
...
# give each signature: html, cannonical internal names, sets of links and priority #'s
sub make_sig_indexes {
    ...
    foreach $sig ($store->alert_field_set($Filter::true,'message')) {
        unless (defined($self->{'signame'}{$sig})) {
            ...
            #$first->debug_print();
            @refs= sort {
                (defined($reflistorder{$a}) ? $reflistorder{$a} : $reflistorder{'_other'}) <=>
                (defined($reflistorder{$b}) ? $reflistorder{$b} : $reflistorder{'_other'})
            } $first->references();
            if (@refs) {
            ...
            my $prio= $first->priority_num();
            my $class= $first->classification_text();
            $self->{'sigprionum'}{$sig}= defined($prio) ? $prio : ($self->{'hiprioisworse'} ? -1 : 99999999);
            $self->{'sigprio'}{$sig}= defined($prio) ? $prio : 'N/A';
            $self->{'sigclass'}{$sig}= defined($class) ? $class : 'N/A';
        }
    }
    my %usedname=();
...
sub sigprio_cmp {
    return $_[0]->{'sigprionum'}{$_[2]} <=> $_[0]->{'sigprionum'}{$_[1]} unless $_[0]->{'hiprioisworse'};
    return $_[0]->{'sigprionum'}{$_[1]} <=> $_[0]->{'sigprionum'}{$_[2]};
}
...
sub output_main_sig_page {
    ...
    select($PAGE);
    my $base;
    $self->{'cur'}{'base'}= $base=&siglist_basepath();
    my $topbase= "$base".$self->{'cur'}{'topbase'};
    ...
    if ($gcount) {
        ...
        print "Earliest alert at ".$self->pretty_time(&earliest_packet($earliest))."<br>\n";
        print "Latest alert at ".$self->pretty_time(&latest_packet($latest))."</p>\n";
    }
    
    my $anompath= $topbase.&anom_sect_dir();
    my $summpath= $topbase.&summ_sect_dir();
    my $acount= defined($self->{'cur'}{'a'}{'store'}) ? $self->{'cur'}{'a'}{'store'}->count($Filter::true) : 0;
    if ($acount) {
        print "<p>The ".$acount." reports from the <A HREF=\"http:\/\/www.silicondefense.com/spice/\">Spade anomaly sensor</A> are in a separate section: <A HREF=\"$anompath/\">visit it</a></p>\n";
    }

    if ($gcount) {
        print "<TABLE BORDER CELLPADDING = 5>\n";
        print "<TR><TD>Priority</TD><TD>Signature (click for sig info)</TD><TD>\# Alerts</TD>".
                    "<TD>\# Sources</TD><TD>\# Dests</TD><TD>Detail link</TD></TR>\n";

        my %sig_count= $store->alert_field_multiset($Filter::true,'message');
        # sort signatures 
        my @sigs= keys %sig_count;
        if ($self->{'cur'}{'sortsigcountfirst'}) { # like of way, sort by # of alerts, then prio
            if ($self->{'cur'}{'mostsigfirst'}) { # most important first
                @sigs= sort { $sig_count{$b} == $sig_count{$a} ? 
                    $self->sigprio_cmp($b,$a) :
                    $sig_count{$b} <=> $sig_count{$a}
                } @sigs;
            } else { # most important last
                @sigs= sort { $sig_count{$a} == $sig_count{$b} ? 
                    $self->sigprio_cmp($a,$b) :
                    $sig_count{$a} <=> $sig_count{$b}
                } @sigs;
            }
        } else { # sort by priority first, then # of alerts
            if ($self->{'cur'}{'mostsigfirst'}) { # most important first
                @sigs= sort { $self->{'sigprionum'}{$b} == $self->{'sigprionum'}{$a} ? 
                    $sig_count{$b} <=> $sig_count{$a} :
                    $self->sigprio_cmp($b,$a)
                } @sigs;
            } else { # most important last
                @sigs= sort { $self->{'sigprionum'}{$a} == $self->{'sigprionum'}{$b} ? 
                    $sig_count{$a} <=> $sig_count{$b} :
                    $self->sigprio_cmp($a,$b) 
                } @sigs;
            }
        ...
        foreach $sig (@sigs) {
            ...
            print "<TR><TD>",$self->{'sigprio'}{$sig},"</TD>";
            print "<TD>",$self->{'sightml'}{$sig};
            if (length($siglinks)) {
                print " $siglinks";
            }
...
# produce the summary pages
sub gen_summ_pages {
    my $self= shift;
    
    $self->{'cur'}{'bgcol'}= $summbgcol;
    $self->{'cur'}{'topbase'}= &summ_sect_basepath();
    $self->output_top_ips('src');
    $self->output_top_ips('dest');
}
...
sub output_per_sig {
    ...
    foreach $sig (keys %sig_count) {
        ...
        my $base;
        $self->{'cur'}{'base'}= $base=&sig_basepath($self->{'signame'}{$sig});
        ...
        print "Earliest such alert at ".$self->pretty_time(&earliest_packet($earliest))."<br>\n";
        print "Latest such alert at ".$self->pretty_time(&latest_packet($latest))."</p>\n";
        ...
        print "<tr><td>Priority: ",$self->{'sigprio'}{$sig},"</td>";
        print "<td colspan=2>Classification: ",$self->{'sigclass'}{$sig},"</td></tr>";
        ...
        print "</table>"; # end of page head table
...
# output the page for the top N source or destination IPs
sub output_top_ips {
    my $self= shift;
    my $end= shift; # 'src' or 'dest'
    my $page_title;
    my $store= $self->{'cur'}{'all'}{'store'};
    my $astore= $self->{'cur'}{'a'}{'store'};
    my $gstore= $self->{'cur'}{'g'}{'store'};
    my $topquant= $self->{'cur'}{'topquant'}; # how many of the IP we were told to show
    my $anom= $astore->count($Filter::true);
    my $end_txt= $end eq 'src' ? 'source' : 'destination';
    my $other_end_txt= $end eq 'src' ? 'destination' : 'source';
    my $which_ip= $end eq 'src' ? 'sip' : 'dip';
    my $other_ip= $end eq 'src' ? 'dip' : 'sip';

    # overall count of packet for src ip
    my %ip_count= $store->packet_field_multiset($Filter::true,$which_ip);

    my @sorted_ips= sort {$ip_count{$b} <=> $ip_count{$a}} (keys %ip_count);
    if (@sorted_ips <= $topquant) { # we have fewer than was requested, so show all
        $endquant= @sorted_ips;
    } else {
        $endquant= $topquant;
        while ($endquant <= $#sorted_ips) { # if following IPs have same # of alerts, show it too
            last if $ip_count{$sorted_ips[$endquant-1]} != $ip_count{$sorted_ips[$endquant]};
            $endquant++;
        }
    }
    my @topips= @sorted_ips[0..($endquant-1)];
    
    my(%g_ip_count,%a_ip_count);
    if ($anom) {
        %g_ip_count= $gstore->packet_field_multiset($Filter::true,$which_ip);
        %a_ip_count= $astore->packet_field_multiset($Filter::true,$which_ip);
    }
    ...
    my $ip;
    foreach $ip (@topips) {
        my $ipfilter= $end eq 'src' ? &Filter::for_anysip($ip) : &Filter::for_anydip($ip);
        $ip_sig_count{$ip}= $gstore->alert_field_set($ipfilter,'message');
        $subsort{$ip}= $ip_sig_count{$ip}+($a_ip_count{$ip}?0.9:0);
        my @otherends= $store->packet_field_set($ipfilter,$other_ip);
        if (@otherends < 3) {
            $otherend_txt{$ip}= join(', ',map(&ip_text($_),@otherends));
        } else {
            $otherend_txt{$ip}= "(".(0+@otherends)." $other_end_txt IPs)";
        }
    }
    
    my $PAGE=$self->open_file($self->{'cur'}{'s'}{'outdir'},$self->top_page($end));
    select($PAGE);

    # Print page head stuff
    $page_title = "List of top $endquant $end_txt IPs in $self->{'cur'}{'insources_titlestr'}";
    my $base;
    $self->{'cur'}{'base'}= $base =&top_basepath($end);
    my $topbase= $base.$self->{'cur'}{'topbase'};
    my $anompath= $topbase.&anom_sect_dir();
    my $sigpath= $topbase.&sig_sect_dir();
    $self->print_page_head($page_title,'summary page',"Top $endquant $end_txt IPs");

    print "<p>This page provides summary information about alerts acquired ".$self->{'cur'}{'insources_html'};
    print "The most active $end_txt IPs are shown.  Rank is determined by the number of alerts with that IP as the $end_txt.  Within a rank, IPs are sorted by # of signatures,",($anom?" then by whether there is an anomaly report,":'')," then by IP number.</p>";
            
    print "<TABLE BORDER CELLPADDING = 5>\n";
    print "<tr align=center><td>Rank</td><td>Total \# Alerts</td>";
    print "<td><strong>",($end eq 'src' ? 'Source' : 'Destination')," IP</strong></td>";
    if ($anom) {
        print "<td>\# Alerts in \# Sigs</td>";
        print "<td>\# Anom Reports</td>";
    } else {
        print "<td>\# Signatures triggered</td>";
    }
    print "<td>",($end eq 'src' ? 'Destinations involved' : 'Originating sources'),"</td></tr>\n";
    
    my $colornum= 0;
    my $color= '#FFFFFF';
    my($i,$j);
    for ($i=0; $i <= $#topips; $i= $j+1) {
        $ip= $topips[$i];
        for ($j=$i+1; $j <= $#topips && ($ip_count{$ip} == $ip_count{$topips[$j]}); $j++) {};
        $j--; # back up to last place where our ip count was same as $i's
        if ($self->{'color_opt'} eq 'rotate') {
            $colornum= ++$colornum % +@HTMLOutput::color;
            $color= $HTMLOutput::color[$colornum];
        }
        @isoips= @topips[$i..$j];
        @isoips= sort { $subsort{$b} == $subsort{$a} ? # sort first by %subsort then by sip
            (&ip_to_num($a) <=> &ip_to_num($b)) :
            ($subsort{$b} <=> $subsort{$a}) } @isoips;
        my $firsttime=1;
        foreach $ip (@isoips) {
            print "<tr bgcolor=\"$color\" align=center>";
            if ($firsttime) {
                print "<td rowspan=\"",0+@isoips,"\">rank #",($i+1),"</td>";
                print "<td rowspan=\"",0+@isoips,"\">",$ip_count{$ip}," alerts</td>";
                $firsttime=0;
            }
            my $ghostpage= $sigpath.$self->host_page($ip,$end);
            if ($anom) {
                my $ahostpage= $anompath.$self->host_page($ip,$end);
                my $gcount= $g_ip_count{$ip};
                my $acount= $a_ip_count{$ip};
                my $url= $gcount ? $ghostpage : $ahostpage;
                print "<td><a href=\"$base".$url."\"><strong>".&ip_text($ip)."</strong></a></td>";
                if ($gcount) {
                    print "<td><a href=\"$base".$ghostpage."\">".$gcount." alerts from ".
                      $ip_sig_count{$ip}." sigs</a></td>";
                } else {
                    print "<td>none</td>";
                }
                if ($acount) {
                    print "<td><a href=\"$base".$ahostpage."\">".$acount." reports</a></td>";
                } else {
                    print "<td>none</td>";
                }
            } else {
               print "<td><a href=\"$base".$ghostpage."\"><strong>".&ip_text($ip)."</strong></a></td>";
               print "<td>".$ip_sig_count{$ip}." signatures</td>";
            }
            print "<td>".$otherend_txt{$ip}."</td></tr>\n";
...
sub gen_anom_pages {
    ...
    $self->{'cur'}{'topbase'}= &anom_sect_basepath();
    ...
    select($PAGE);
    $self->{'cur'}{'base'}= &anomindex_basepath();
...
sub output_all_anom_page {
    ...
    $self->{'cur'}{'base'}= &anomall_basepath($sortbytext);
...
sub output_anom_src_page
{
    ...
    my $base;
    $self->{'cur'}{'base'}= $base= &anomsrcs_basepath();
...
sub output_anom_dest_page
{
    ...
    my $base;
    $self->{'cur'}{'base'}= $base= &anomdests_basepath();
...
sub output_anom_pagelist {
    ...
    print "<LI><A HREF=\"$base".$self->anomall_page('score')."\">All alerts sorted by score</a>\n";
    print "<LI><A HREF=\"$base".$self->anomall_page('time')."\">All alerts sorted by time</a>\n";
    print "<LI><A HREF=\"$base".$self->anomsrcs_page()."\">A list of the $srccount source IP addresses in the alerts</a>\n";
    print "<LI><A HREF=\"$base".$self->anomdests_page()."\">A list of the $destcount destination IP addresses in the alerts</a>\n";
    print "</UL>\n";
}
...
sub output_per_host {
    ...
    $self->{'cur'}{'base'}= $host_basepath= &host_basepath($ip,$end);
    ...
    if ($thresh == 0 || $num_alerts <= $thresh) {
        ...
        print "<UL><LI><A HREF=\"$host_basepath$all_file\">view the whole listing</A>\n";
        ...
        foreach ($first=1; $first <= $num_alerts; $first+=$thresh) { # segment the alerts
            ...
            print "<tr><td><A HREF=\"$host_basepath$range_file\">$first to $last</A></td><td>",$self->pretty_time(&earliest_packet($early)),"</td><td>",$self->pretty_time(&latest_packet($late)),"</td></tr>\n";
            ...
            print "<hr>";
            my $nav= "Go to: ".
                ($first == 1?'':"<A HREF=\"$host_basepath".$self->host_page($ip,$end,($first-$thresh))."\">previous range</A>, ").
                (($first+$thresh >= $num_alerts)?'':"<A HREF=\"$host_basepath".$self->host_page($ip,$end,($first+$thresh))."\">next range</A>, ").
                " <A HREF=\"$host_basepath$all_file\">all alerts</A>, <A HREF=\"$host_basepath$ip_file\">overview page</A>";
            print $nav;
        ...
        $self->print_page_head("All $num_alerts $al_descr",$page_type,$end_ip);
        $self->output_per_host_header($alerts[0],$alerts[$#alerts],$num_alerts);
        my $nav= "Go to: <A HREF=\"$host_basepath$ip_file\">overview page</A>";
        print $nav;
        $self->output_alert_table(\@alerts);
        print $nav;
        $self->print_page_foot();
...
sub output_per_host_header
{
    ...
    print "<br>Earliest: ".$self->pretty_time(&earliest_packet($early))."<br>\n";
    print "Latest: ".$self->pretty_time(&latest_packet($late))."\n<P>";
    ...
    my(%path);
    $path{'g'}= $base.$self->{'cur'}{'topbase'}.&sig_sect_dir();
    $path{'a'}= $base.$self->{'cur'}{'topbase'}.&anom_sect_dir();
    ...
    my $insources_str= $self->{'cur'}{'insources_str'};
    my $insources_str_noinfilter= $self->{'cur'}{'insources_str_noinfilter'};
    my $in_filter_used= $insources_str ne $insources_str_noinfilter;
    if ($self->{'cgiavail'} && ($ip ne '*undef*')) {
        print "<td colspan=$cols align=center>";
        print "<A HREF=\"$self->{'cgi_dir'}/text4sel.pl?".
            join('&',"end=$end","ip=$ip","include=$type",
                'sources='.&url_encode($insources_str_noinfilter)
            ).'"';
        print ">Fresh grab of all $typefulldescr (as text)</A>";
        if ($in_filter_used) {
            print " <BR><A HREF=\"$self->{'cgi_dir'}/text4sel.pl?".
                join('&',"end=$end","ip=$ip","include=$type",
                    'sources='.&url_encode($insources_str)
                ).'"';
            print ">(or with current input filters in place)</A>";
        }
        print "</td></tr>\n";
    }
    ...
    if (defined($self->{'sisr_config'}) && ($ip ne '*undef*')) {
        ...
        print "<td colspan=$colwidth align=center>";
        print "<A HREF=\"$self->{'cgi_dir'}/sel_to_add.pl?".join('&',"configfile=$encconfig","end=$end","ip=$ip",'include='.($type eq 'g'?'g':'a'),'sources='.&url_encode($insources_str_noinfilter)).'"',$self->target('sisrwin'),">$linktext</A>";
        print " <A HREF=\"$self->{'cgi_dir'}/sel_to_add.pl?".join('&',"configfile=$encconfig","end=$end","ip=$ip",'include='.($type eq 'g'?'g':'a'),'sources='.&url_encode($insources_str)).'"',$self->target('sisrwin'),">(use input filter)</A>" if $in_filter_used;
        print "</td></tr>\n";
        if ($showcreate2) {
            $linktext= "Add some of both type alerts $endip to a labeled set";
            print "<tr bgcolor=\"#D5E2CE\"><td colspan=$colwidth align=center>";
            print "<A HREF=\"$self->{'cgi_dir'}/sel_to_add.pl?".join('&',"configfile=$encconfig","end=$end","ip=$ip",'include=ga','sources='.&url_encode($insources_str)).'"',$self->target('sisrwin'),">$linktext</A>";
            print " <A HREF=\"$self->{'cgi_dir'}/sel_to_add.pl?".join('&',"configfile=$encconfig","end=$end","ip=$ip",'include=ga','sources='.&url_encode($insources_str)).'"',$self->target('sisrwin'),">(use input filter)</A>" if $in_filter_used;
            print "</td></tr>\n";
        }
...
sub alert_as_html { 
    ...
    my $text= $alert->as_text();
    my $sig= $alert->message();
    my $type= $alert->type();
    if ($type eq 'spade') {
        if ($text !~ s/(Spade:\s*)([^:]+)(:\s*[^:]*:\s*)(\d+\.\d+)/$1<strong>$2<\/strong>$3<strong>$4<\/strong>/) {
            # didn't match new format
            $text =~ s/^(.*)(\d+\.\d+)/$1<strong>$2<\/strong>/;
        }
    } elsif ($type eq 'spp_portscan2') {
        $text =~ s/src:\s*(\S+)\s+dst:\s*(\S+)\s+sport:\s*(\S+)\s+dport:\s*(\S+)/$1:$3 -> $2:$4/;
        $text =~ s/src:\s*(\S+)\s+dst:\s*(\S+)/$1 -> $2/;
    } elsif (defined($sig)) {
        my $sigre= $sig;
        $sigre =~ s/([^\w ])/\\$1/g;
        $sigurl= $self->sig_page($self->{'signame'}{$sig});
        $text =~ s/($sigre)(\s*\[\*\*\])/<a href=\"$base$sigurl\">$1<\/A>$2/;
    }
    my ($sep,$more);
    ($text,$sep,$more)= split(/(ORIGINAL\s*\w*\s*DUMP)/i,$text); #only link to IPs alerted on
    if ($text =~ /:\d+ \->/) {
        my $packet= &earliest_packet($alert);
        my $proto= defined($packet) ? $packet->protocol() : 'ANY';
        $proto= 'ANY' if (!defined($proto) || ($proto ne 'TCP' && $proto ne 'UDP')); # shouldn't happen
        $text =~ s/(\d+\.\d+\.\d+\.\d+):(\d+) ->/"<A HREF=\"$base".$self->host_page($1,'src')."\">$1<\/A>:<A HREF=\"http:\/\/www.portsdb.org\/bin\/portsdb.cgi?portnumber=$2&protocol=$proto\" $newwindow>$2<\/A> ->"/e;
        $text =~ s/->(\s*)(\d+\.\d+\.\d+\.\d+):(\d+)/"->$1<A HREF=\"$base".$self->host_page($2,'dest')."\">$2<\/A>:<A HREF=\"http:\/\/www.portsdb.org\/bin\/portsdb.cgi?portnumber=$3&protocol=$proto\" $newwindow>$3<\/A>"/e;
    } else {
        $text =~ s/(\d+\.\d+\.\d+\.\d+)(.*)->/"<A HREF=\"$base".$self->host_page($1,'src')."\">$1<\/A>$2->"/e;
        $text =~ s/->(\s*)(\d+\.\d+\.\d+\.\d+)/"->$1<A HREF=\"$base".$self->host_page($2,'dest')."\">$2<\/A>"/e;
    }
    if (defined($sep)) {
        $more =~ s/(\d+\.\d+\.\d+\.\d+):(\d+)\s*->/$1:<A HREF=\"http:\/\/www.portsdb.org\/bin\/portsdb.cgi?portnumber=$2\" $newwindow>$2<\/A> ->/;
        $more =~ s/->\s*(\d+\.\d+\.\d+\.\d+):(\d+)/-> $1:<A HREF=\"http:\/\/www.portsdb.org\/bin\/portsdb.cgi?portnumber=$2\" $newwindow>$2<\/A>/;
        $text.= $sep.$more;
    }
    $text =~ s/[\n\r]+/<br>/g;
...
sub get_alert_logpage {
    my($self,$alert)=@_;
    my $type= $alert->type();
    return undef if $type eq 'spp_portscan' || $type eq 'spp_portscan2'; # no log for portscan alerts
    ...
    my $dir= $self->{'log_base'}.$ip.'/';
    if ($proto eq 'ICMP') {
        my $ICMP_type= $pkt->flags();
        return undef unless defined($ICMP_type);
        unless (defined($ICMP_text_to_filename{$ICMP_type}) || ($self->{'icmptypetextwarnedabout'}{$ICMP_type} > 1)) {
            print STDOUT "Warning: unknown ICMP type text \"$ICMP_type\"; corresponding log links will not exact\n";
            print STDOUT "  Future warnings about above are being supressed\n" if $self->{'icmptypetextwarnedabout'}{$ICMP_type} > 1;
            $self->{'icmptypetextwarnedabout'}{$ICMP_type}++;
        }
        my $filename= $ICMP_text_to_filename{$ICMP_type};
        return $dir unless defined($filename);
        return $dir.$filename.$self->{'logfileext'};
    } elsif (defined($sport) && defined($dport)) {
        my $prototext= ($proto eq 'UDP')?'UDP':'TCP';
        return $dir.$prototext.$self->{'logfileprototerm'}.$port1."-".$port2.$self->{'logfileext'};
    } else {
        return undef;
    }
...
sub print_ip_lookup
{
    ...
    my $host= undef;
    $host= gethostbyaddr(inet_aton($ip), AF_INET) if $self->in_dnsnet($ip);
    
    my $target= $self->target('lookup');
    print "<tr><td rowspan=3 bgcolor=\"#E0CDD0\" align=\"center\">$ip</td>\n";
    print "<td rowspan=3 bgcolor=\"#E0CDD0\">($host)</td>\n" if defined $host;
    ...
    print "<td><a href=\"http://www.ripe.net/perl/whois".
...
#    print "<td><a href=\"http://wwwnet.princeton.edu/cgi-bin/dnslookup.pl?advanced_output=on&target=$ip\"$target>Princeton</a></td>\n";
    print "</tr>\n<tr>\n";
    print "<td>More lookup links:</td>\n";
    print "<td><a href=\"http://www.dshield.org/ipinfo.php?ip=$ip\"$target>Dshield</a></td>\n";
    print "<td><a href=\"http://www.samspade.org/t/lookat?a=$ip\"$target>Sam Spade</a></td>\n";
    print "</tr>\n";
...
sub print_top_nav_links {
    my $self= shift;
    my $gcount= defined($self->{'cur'}{'g'}{'store'}) ? $self->{'cur'}{'g'}{'store'}->count($Filter::true) : 0;
    my $acount= defined($self->{'cur'}{'a'}{'store'}) ? $self->{'cur'}{'a'}{'store'}->count($Filter::true) : 0;
    my $base= $self->{'cur'}{'base'}.$self->{'cur'}{'topbase'};
    my $sigbase= $base.&sig_sect_dir();
    my $summbase= $base.&summ_sect_dir();
    my $anombase= $base.&anom_sect_dir();
    my $topquant= $self->{'cur'}{'topquant'};

    print "<p><center><table border><tr>";
    my $url= $sigbase.$self->siglist_page();
    print "<td><a href=\"$url\">Signature section ($gcount)</a></td>";
    $url= $anombase.$self->anomindex_page();
    print "<td><a href=\"$url\">Anomaly report section ($acount)</a></td>" if $acount;
    $url= $summbase.$self->top_page('src');
    print "<td><a href=\"$url\">Top $topquant source IPs</a></td>";
    $url= $summbase.$self->top_page('dest');
    print "<td><a href=\"$url\">Top $topquant dest IPs</a></td>";
...
sub earliest_packet {
    my $alert= shift;
    my @pkts= $alert->packets();
    return $pkts[0] if @pkts == 1; # currently most common case; handle it quickly
...
$pkt->debug_print($pkt,STDOUT) unless defined($todtext);
    ...
    if ($self->{'gmt'}) {
        my @gmt=split(/:/, $tod);
        my $gmt = timegm($gmt[2], $gmt[1], $gmt[0], $pkt->day(), $pkt->month()-1, (($pkt->year()> 100)?$pkt->year()-1900:$pkt->year()));
      
        my($s, $m, $h, $d, $mn, $y) = localtime($gmt);
        $mn++;
        $mn = "0$mn" if $mn < 10;
        $d = "0$d" if $d < 10;
      
        my $date= $self->{'ymd'} ? ($y+1900) ."/".$mn."/".$d : $mn."/".$d."/".($y+1900);
        return "<b>$h:$m:$s</b>".
            (defined($secfrac)&&($secfrac ne '')?".$secfrac":'').
            " <i>on ".$date."</i>";
    } else { 
        my $date= $self->{'ymd'} ? $pkt->year()."/".$pkt->month()."/".$pkt->day() : $pkt->month()."/".$pkt->day()."/".$pkt->year();
        return "<b>$tod</b>".
            (defined($secfrac)&&($secfrac ne '')?".$secfrac":'').
            " <i>on ".$date."</i>";
    }
...
sub in_dnsnet {
    my($self,$ip)= @_;
    return 0 unless defined($self->{'dnsaddr'});
    my $ipaddr= &bytenums2bits(split('\.',$ip));

    return ($ipaddr & $self->{'dnsmask'}) == $self->{'dnsaddr'};
}

##############################################################################

# open a file in the given base directory, creating subdirectories as needed
sub open_file {
    my ($self,$basedir,$file,$isbinary)= @_;
    my $fh= $fhnonce++;
    my $dirsep= $self->{'dirsep'};
    my $dirsepre= '[\\'.$dirsep.'\/]';  # split on dirsep or /
    my @okaypath= (split($dirsepre,$basedir),split($dirsepre,$file));
    my @notokay= ();
    my ($dir,$path);
#print STDOUT "opening file $file in $basedir (\@okaypath is ",join('--',@okaypath),")\n";
    do {
        unshift(@notokay,pop(@okaypath));
        $dir= join($dirsep,@okaypath);
    } while (@okaypath && ! -d $dir);
    while (@notokay) {
        push(@okaypath,shift(@notokay));
        $path= join($dirsep,@okaypath);
        if (!@notokay) { # just file left
            open($fh,">$path") || die("Couldn't create file $file in $basedir\n");
...
# lookup the base dir of a section and how to get from there to the overall base directory
sub sig_sect_dir {
    return '';
}
sub sig_sect_fsdir {
...
sub sig_sect_basepath {
    return '';
}
sub anom_sect_dir {
    return 'anomrep/';
}
sub anom_sect_basepath {
    return '../';
}
sub summ_sect_dir {
    return '';
}
sub summ_sect_basepath {
    return '';
}

# lookup the name of a page and how to get from there to the section base directory
sub siglist_page {
    return "index.$_[0]->{'html'}";
}
sub siglist_basepath {
    return '';
}
sub sig_page {
    my($self,$signame)= @_;
...
sub anomall_basepath {
    return '';
}
sub anomsrcs_page {
    return "srcips.$_[0]->{'html'}";
}
sub anomsrcs_basepath {
    return '';
}
sub anomdests_page {
    return "destips.$_[0]->{'html'}";
}
sub anomdests_basepath {
    return '';
}
sub top_page {
    my($self,$dir)= @_;
    return "top".$dir."s.$_[0]->{'html'}";
}
sub top_basepath {
    return '';
}
...
sub clear_dir {
    ...
    if (opendir($fh,$dirpath)) {
        while ($file=readdir($fh)) {
            ...
            } elsif ($file =~ /^((src|dest)\d+\.\d+\.\d+\.\d+.*|(sig.*)|top.*|allbyscore|allbytime|srcips|destips|index)\.$self->{'html'}/) {
